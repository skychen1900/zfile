...
public interface ShortLinkConfigRepository
		extends JpaRepository<ShortLinkConfig, Integer>, JpaSpecificationExecutor<ShortLinkConfig> {
	...
	@Query(nativeQuery = true, value = " select * from short_link where " + " key like concat('%', :key,'%') "
			+ " and url like concat('%', :url,'%') " + " and (:dateFrom is null or create_date >= :dateFrom"
			+ " and (:dateTo is null or create_date <= :dateTo) ", countQuery = " select count(1) from short_link where "
					+ " key like concat('%', :key,'%') " + " and url like concat('%', :url,'%') "
	...
	// " and (:dateFrom is null or date_format(create_date, '%Y-%m-%d') >=
	// date_format(:dateFrom, '%Y-%m-%d'))" +
	// " and (:dateTo is null or date_format(create_date, '%Y-%m-%d') <=
	 ...
	 * 更新驱动器 ID
	 *
	 * @param updateSubPath 原路径部分名称
	 *
	 * @param newSubPath    修改后路径部分名称
	 */
	@Modifying

...
public class OneDriveConfig {
	...
	/**
	 * OneDrive 请求 RestTemplate, 会在请求头中添加 Bearer: Authorization {token} 信息, 用于 API
	 * 认证.
	 */
	@Bean
	public RestTemplate oneDriveRestTemplate() {
		...
		ClientHttpRequestInterceptor interceptor = (httpRequest, bytes, clientHttpRequestExecution) -> {
			...
			Integer driveId = Integer.valueOf(((List<String>) headers.get("driveId")).get(0).toString());

			StorageConfig accessTokenConfig = storageConfigService.findByDriveIdAndKey(driveId,
					StorageConfigConstant.ACCESS_TOKEN_KEY);

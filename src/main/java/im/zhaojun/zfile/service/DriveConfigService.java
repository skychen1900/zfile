...
public class DriveConfigService {
	...
	public DriveConfigDTO findDriveConfigDTOById(Integer id) {
		DriveConfig driveConfig = driverConfigRepository.findById(id).map(config -> config).orElse(null);
	...
	public void updateId(Integer updateId, Integer newId) {
		...
		String updateSubPath = StringUtils.concatUrl(StringUtils.DELIMITER_STR, ZFileConstant.DIRECT_LINK_PREFIX,
				String.valueOf(updateId));
		String newSubPath = StringUtils.concatUrl(StringUtils.DELIMITER_STR, ZFileConstant.DIRECT_LINK_PREFIX,
	...
	/**
	 * 刷新指定 key 的缓存: 1. 清空此 key 的缓存. 2. 重新调用方法写入缓存.
	 *
	 * @param driveId 驱动器 ID
	 *
	 * @param key     缓存 key (文件夹名称)
	 */
	public void refreshCache(Integer driveId, String key) throws Exception {
		...
		BaseFileService baseFileService = driveContext.get(driveId);
